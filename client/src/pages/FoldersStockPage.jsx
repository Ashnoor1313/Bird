...
export const FoldersStockPage = () => {
  ...
  // TanStack Query: Category Hub Data (Instant Cached with SWR)
  const { data, isLoading: loading, isFetching, refetch: loadFoldersData } = useCategoryHubData(
    activeBusinessId,
    'Folders',
    activeLocationId
  );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const url = `/api/products/export/excel?businessId=${activeBusinessId}&scope=category&categoryName=Folders${locParam}`;
      ...
      a.href = downloadUrl;
      a.download = `BIRD_Folders_Stock_${new Date().toISOString().split('T')[0]}.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      addToast('Folders stock exported to Excel!', 'success');
    } catch (err) {
    ...
    try {
      ...
      const res = await fetch('/api/stock/adjust', {
        ...
        body: JSON.stringify({
          ...
          note: `Folder stock update (${adjustType === 'ADD' ? '+' : '-'}${qtyNumber})`,
        }),
      });
  ...
  const handleAddFolderSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        addToast(`Folder model "${manualName}" added!`, 'success');
        ...
        const errJson = await res.json();
        addToast(errJson.error || 'Failed to add folder model', 'error');
      }
    } catch (err) {
      addToast('Error adding folder model', 'error');
    } finally {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errJson = await res.json();
        addToast(errJson.error || 'Failed to update folder', 'error');
      }
    } catch (err) {
  ...
  // Add Customer Submit (Scoped to Folders in this Store)
  const handleAddCustomerSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        addToast(`Customer "${newCustName}" added to Folders accounts!`, 'success');
    ...
    <div className="p-3 sm:p-6 space-y-4 max-w-7xl mx-auto pb-24 lg:pb-8">
      ...
      <div className="bird-card p-4 sm:p-5 bg-gradient-to-br from-white to-zinc-50 border border-zinc-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-2xl bg-zinc-900 text-white flex items-center justify-center shadow-xs shrink-0">
            <Smartphone className="w-6 h-6" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold tracking-tight text-zinc-900">
                {isGodown ? 'FOLDERS STOCK' : `${locationLabel} — FOLDERS`}
              </h1>
            ...
            <p className="text-xs text-zinc-500 font-medium mt-0.5">
              {isGodown
                ...
                : 'Dedicated Folder sales, customer accounts, store stock & payments'}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search folder model (e.g. A15, 14 pro, Redmi Note 10)..."
                value={search}
          ...
          {filteredProducts.length === 0 ? (
            <div className="bird-card p-12 text-center space-y-3">
              <Smartphone className="w-10 h-10 text-zinc-300 mx-auto" />
              <div className="text-sm font-bold text-zinc-800">No folder models match</div>
              <button onClick={() => setShowManualModal(true)} className="btn-primary text-xs py-2 px-3">
                <Plus className="w-3.5 h-3.5" />
                <span>+ Add Folder Model</span>
              </button>
            </div>
          ) : (
      ...
      {!isGodown && activeTab === 'sales' && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-sm text-zinc-900">Folder Sales Bills ({bills.length})</h3>
            <button
              onClick={() => navigate('/sales?action=new&category=folders')}
          ...
          {bills.length === 0 ? (
            <div className="bird-card p-12 text-center space-y-2">
              ...
              <p className="text-xs font-bold text-zinc-700">No folder bills yet in {locationLabel}</p>
            </div>
          ) : (
            <div className="bird-card overflow-hidden">
      ...
      {!isGodown && activeTab === 'customers' && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-sm text-zinc-900">Folder Customers in {locationLabel} ({categoryCustomers.length})</h3>
            <button
          ...
          {categoryCustomers.length === 0 ? (
            <div className="bird-card p-12 text-center space-y-2">
              ...
              <p className="text-xs font-bold text-zinc-700">No folder customers registered in {locationLabel} yet</p>
      ...
      {!isGodown && activeTab === 'payments' && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-sm text-zinc-900">Folder Payments in {locationLabel}</h3>
            <button
          ...
          {categoryPayments.length === 0 ? (
            <div className="bird-card p-12 text-center space-y-2">
              ...
              <p className="text-xs font-bold text-zinc-700">No folder payment records in {locationLabel}</p>
            </div>
          ) : (
            <div className="bird-card overflow-hidden">
              <table className="enterprise-table">
        ...
        <div className="fixed inset-0 z-50 bg-zinc-950/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-zinc-200 w-full max-w-sm rounded-2xl shadow-xl p-5 space-y-4 animate-fade-in">
            ...
            <form onSubmit={handleAddFolderSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-zinc-700 mb-1">Folder Model Name *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Samsung, Apple, Vivo, Realme"
                  value={manualBrand}
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-zinc-100">
                ...
                <button type="submit" disabled={addingManual} className="btn-primary font-bold">
                  {addingManual ? 'Saving...' : 'Save Folder'}
                </button>
              </div>
            </form>
          </div>
      ...
      {editingProduct && (
        <div className="fixed inset-0 z-50 bg-zinc-950/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-zinc-200 w-full max-w-sm rounded-2xl shadow-xl p-5 space-y-4 animate-fade-in">
            <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
              <h3 className="font-bold text-sm text-zinc-900">Edit Folder Model</h3>
      ...
      {showAddCustomerModal && (
        <div className="fixed inset-0 z-50 bg-zinc-950/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-zinc-200 w-full max-w-sm rounded-2xl shadow-xl p-5 space-y-4 animate-fade-in">
            <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
              <h3 className="font-bold text-sm text-zinc-900">+ Add Folder Customer</h3>
            ...
            <form onSubmit={handleAddCustomerSubmit} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Rahul Mobile / Repair Point"
                  value={newCustName}
...
export default FoldersStockPage;
